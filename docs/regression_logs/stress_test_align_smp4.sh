...
KERNEL=build/kernel_align.elf
N=30
CRASH_COUNT=0
CLEAN_COUNT=0
UNCLEAR_COUNT=0
LOGDIR=stress_logs_align_smp4
mkdir -p "$LOGDIR"

for i in $(seq 1 "$N"); do
    echo "=== Run $i/$N ==="
    script -qec "timeout 40 $QEMU -M virt -cpu cortex-a53 -smp 4 -nographic -device dma-accel -kernel $KERNEL" \
